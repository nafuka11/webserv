...
private:
    ...
    template <typename T>
    bool isDuplicateAutoindex(T &config_obj);
...
{
    if (isDuplicateAutoindex(config_obj))
    {
...
template <typename T>
bool ConfigParser::isDuplicateAutoindex(T &config_obj)
{
    if (config_obj.autoindex() != MainConfig::DIRECTIVE_UNSET_STR)
    {
        return true;
    }
    return false;
